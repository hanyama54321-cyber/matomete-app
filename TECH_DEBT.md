...
- **iOS PWA standaloneのタブバー下白帯(修正済み)**: `#app`が`height:100dvh`依存だったため、iOS standaloneでホームインジケータ領域を含まない高さになりアプリシェル全体が短くなっていた。`#app`を`position:fixed;inset:0`(`viewport-fit=cover`前提)に変更して解消。`#bottom-nav`は`position:fixed`に戻していない(v0.2.3で揺れ問題により撤去した経緯があるため)。**iOS実機での確認は完了済み(v0.4の実機確認時、PWA下部の白帯は問題なし)。**
...
防ぐための定数。現在の値は`2026-07-30T03:30:00+09:00`(v0.4のリリース時刻)。

**更新が必要なのは「この仕組みを新たに載せる」リリースだけ**で、通常のリリースでは触らない。
この定数は`state/badges`が未作成のユーザーを初期化するときの足切り値としてのみ使われるため、
既にログイン済みのユーザーには影響しないため(v0.4.1・v0.4.2では新着判定を変えていないので
更新していない。マージ前に差分ゼロであることを確認する運用にしている)。

`since`は`max(BADGE_EPOCH, users.createdAt ?? Auth.metadata.creationTime)`で決まる。
...
## 15. チャットの日付セパレータは「今日/昨日」が日付をまたいでも自動更新されない(v0.4.3・意図的に未対応)

v0.4.3でチャットに日付セパレータを入れた([index.html](index.html)の`renderChat()`)。
ラベルの「今日」「昨日」は**描画時点の`new Date()`と比較して決まる相対表記**のため、
**アプリを開いたまま深夜0時をまたぐと、次の再描画まで「今日」が古い日付を指したまま残る**。

例: 23:50に開いたチャットを0:10まで開きっぱなしにすると、前日のメッセージ群の上に
「今日」と出たままになる(正しくは「昨日」)。

### 対応しない判断(2026-08-14)

配送業務は深夜帯もあるため発生し得るが、以下の理由で今回は記録のみとした。

- **新しいメッセージが1通でも届けば`onSnapshot`→`renderChat()`が走り、自然に直る**。
  チャンネル切替・タブ切替でも直る
- 日付をまたぐ瞬間にタイマーで再描画を回す方法はあるが、**割に合わない**。
  タイマーの管理(チャンネル切替・ログアウト・`visibilitychange`での解除)が増え、
  リスナーのdetach漏れと同種の事故を作り込みやすい
- 誤りは「今日/昨日」の2ラベルのみで、それ以前の日付(`8月1日(土)`形式)は絶対表記のため
  影響を受けない

### 直す場合に触る場所

`fmtChatDateLabel()`が`new Date()`を直接参照している1箇所のみ。再描画さえ走れば正しくなるため、
**ラベル生成側ではなく「日付が変わったら`renderChat()`を1回呼ぶ」形で入れること**。
