...
`feature/v0.4.2-onclick-sweep`は`develop`へマージ済みです(削除して構いません)。
v0.4.1がmain未マージの時期にv0.4.1のマージへ未検証コードが混入するのを避けるため、
一時的にfeatureブランチで作業していたものです。

## 次のセッションで最初に確認すること

**未確認の宿題はありません。** v0.4.2までのリリースと実機確認は完了しています。
次の作業は下記「次にやるとよさそうなこと」から選んでください。
...
1. **[最優先]** マニュアル3点の追随。v0.4.1〜v0.4.2で赤点のサイズが変わったため、早見表の
   端末図解(①ベルの赤点 ②タブの赤点 ③必読の目印)とドライバー用ガイドの画面レプリカが
   実物とずれています。実物の値は**ベル芯12px・実効16px / タブと未読は芯10px・実効14px /
   赤`#FF3B30` / 未読は青`#1A56C4` / リング2px**。意味は変わっていないため緊急性は低いです。
