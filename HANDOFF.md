...
- **アプリバージョン**: **v0.4.2(`feature/v0.4.2-onclick-sweep`ブランチ)**。
  本番公開中は引き続き**v0.4**(マージコミット`cf15ad2`、タグ`v0.4`)。
  マージ・タグ付けはすべて南野さんが実施します。
- **v0.4の実機確認は完了済み**(ベル・タブの新着マーク・通知センター・PWA下部の白帯、いずれも問題なし)。

### ⚠️ ブランチ構成に注意(v0.4.1が未マージのため3段になっています)

| ブランチ | 内容 | 状態 |
|---|---|---|
| `main` | v0.4 | 本番公開中(`cf15ad2`、タグ`v0.4`) |
| `develop` | v0.4.1 | **未マージ**。`77ab72a`で凍結中、実機確認待ち |
| `feature/v0.4.2-onclick-sweep` | v0.4.2 | 本セッションの作業。`develop`から分岐 |

**v0.4.2をfeatureブランチで作業したのは、v0.4.1が`main`未マージだったためです。**
`develop`に積むと、v0.4.1のマージに未検証のv0.4.2コードが混入します。
**v0.4.1のマージ完了後に、featureブランチを`develop`へ取り込んでください。**

想定タグ: v0.4.1のロールバック用が`v0.4-pre-badge-fix`、v0.4.2のロールバック用が
`v0.4.1-pre-onclick-sweep`(いずれも作成は南野さん)。

## 次のセッションで最初に確認すること

**v0.4.1・v0.4.2とも実機確認がまだです。** 実装とブラウザ実測までは完了していますが、
...
**あわせて`--badge-dot-bell`の最終値を確定すること。** 現在は10pxで、変更する場合は
[index.html](index.html)の`:root`にある同定数1箇所を書き換えるだけで済みます。
デスクトップの原寸レプリカでは判断できない(PWAスタンドアロンの実機と見え方が異なる)ため、
実機で判断する方針です。

**v0.4.2の確認項目:**

6. 周知タグ・KYT選択肢・班名に`"`や`'`を含めても、タップが効く
7. タグフィルタ・KYTの選択・チャンネル切替が従来どおり動く(デグレ確認)

> **⚠️ 暫定の回避策(2件)——リリース・実機確認が済んだら解除してください**
>
> **どちらもv0.4.2で修正済みですが、本番へ反映されるまでは有効です。**
> マージ → Pages反映 → 実機確認まで完了した時点で、この節ごと削除して構いません。
>
> **(1) 運用: 周知タグ・KYT選択肢・班チャンネル名に、ダブルクォート`"`とシングルクォート`'`を使わないこと。**
> 入力できるのは管理者のみのため運用で回避できます。詳細は[TECH_DEBT.md](TECH_DEBT.md)項目12。
>
> **(2) 実装: `index.html`の`.reach-mini-public:hover`付近に新しいCSSを追加しないこと。**
> セレクタを失った宣言ブロックがあり、そこから後続のルールが**無言で飲み込まれます**。
> 詳細は[TECH_DEBT.md](TECH_DEBT.md)項目14。

## 本セッション: v0.4.2 インラインonclickの横展開・CSS宣言ブロック除去(featureブランチ)
...
## 前セッション: v0.4.1 手順書編集の保存不具合修正・新着ドットの視認性強化(develop・未マージ)
...
1. **[最優先]** v0.4.1のマージと実機確認(上記「次のセッションで最初に確認すること」の1〜5)、
   および`--badge-dot-bell`の最終値の確定。
2. **[最優先の次]** v0.4.1のマージ完了後、`feature/v0.4.2-onclick-sweep`を`develop`へ取り込み、
   v0.4.2として同じ流れでリリースする(確認項目は6〜7)。
3. リリース後: マニュアル3点の追随(ドットのサイズ変更分。意味は変わらないため緊急性は低い)。
   あわせて、HANDOFF冒頭の「暫定の回避策(2件)」の節を削除する。
