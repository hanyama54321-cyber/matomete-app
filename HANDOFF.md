...
- **v0.4 / v0.4.1 / v0.4.2 は実機確認完了**(問題なし)。**v0.4.3は実機確認待ちです**(下記)。
...
**v0.4.3(チャットの日付セパレータ)の実機確認がまだです。** マージ・タグ付け・Pages反映確認まで
完了しており、iOS実機での確認だけ南野さんの担当として残っています。結果を聞いてください。

**確認項目:**

1. チャットで日付が変わる位置にセパレータが出る(今日 / 昨日 / `8月1日(土)` / `2025年12月31日(水)`)
2. 同じ日のメッセージの間には出ない
3. メッセージを送信した直後にセパレータが二重に出たり、変な位置に出たりしない
4. 既存の時刻表示・チャンネル切替・他班チャンネルの入力欄無効化にデグレがない
5. チャンネルを開いたとき、これまでどおり最下部まで自動スクロールする

**あわせてセパレータの色・サイズの最終判断をお願いする位置づけです。** 現在は
`--surface2`(背景)/ `--text-sub`(文字)の流用で、11px・太字・角丸999pxのピル。
デスクトップの原寸レプリカは実機と見え方が違い判断材料にならないことが
`--badge-dot-bell`のときに分かっているため、**比較ページは作らず実機で判断する方針**です。
調整はCSS数行なので次のリリースで直せます([index.html](index.html)の`.chat-date-sep`)。

> **⚠️ 実機確認の結果報告まで`develop`に新しい変更を積まないでください。**
> 本番に入っているものを一意に保つためです。

**それ以外に未確認の宿題はありません。** ここから先は新規開発を止めて運用に集中する方針です
(v0.4.3は南野さんの個別指示で実施)。指示があるまで着手しないでください。
...
| (次) | バージョン0.4.3・CHANGELOG・TECH_DEBT項目15・HANDOFFの更新 |
...
1. v0.4.3の残タスク: 実機確認の結果を受けて、**日付セパレータの色・サイズを確定する**
   ([index.html](index.html)の`.chat-date-sep`。CSS数行)。「ここから未読」ラインと
   `position: sticky`での上部固定も、実機を見てから改めて判断する位置づけです。
