...
`develop`にはドキュメントのみの差分(この引き継ぎ書の更新)が載っており、**mainには
マージしていません**。`main`を`v0.4.2`タグと一致した状態に保つためで、次のリリースに同梱します。
...
2026-08-03のマニュアル調査中に見つけたものです。どちらもDrive上の配布物で、リポジトリの
コードとは無関係です。

1. **[優先度: 高] 全社展開のご案内(Slides)が「LINEでログイン」のまま。**
   LINE連携はセキュリティ上の理由で**v0.2で見送りが決定**しており([TECH_DEBT.md](TECH_DEBT.md)項目6)、
   これは誤情報です。実際は乗務員コード+パスワードによるFirebase Authentication。
   同じスライドに「パスワードも覚えなくてOK」「新しいパスワードは不要」という記述もあり、
   初回パスワード変更の周知と矛盾します。**他拠点展開の検討が動いているため優先度が高い**です。
   - Drive: `安全配送まとめてアプリ/安全配送まとめてアプリ_全社展開のご案内_社内向け`(Google スライド)

2. **管理者向け運用マニュアル(Googleドキュメント、v0.2表記)の「6. 今後の予定」が古い。**
...
## 本セッション: v0.4.2 インラインonclickの横展開・CSS宣言ブロック除去(mainへ本公開済み)
...
## 前セッション: v0.4.1 手順書編集の保存不具合修正・新着ドットの視認性強化(mainへ本公開済み)
